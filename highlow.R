highlow <- function(x, period = "week", n = 60) {
  ...
  # 计算月度收益率及收益率差异,相当于计算1阶2阶导数
  period_df <- ungroup(period_df) %>%
    mutate(profit = adjusted - lag(adjusted),
           diff = profit - lag(profit))
  
  # 高点为本月的 diff > 0,次月的diff < 0,低点反之
  period_df <- mutate(period_df, 
                    if_high = if_else(profit > 0 & lead(profit) < 0, TRUE, FALSE), 
                    if_low = if_else(profit < 0 & lead(profit) > 0, TRUE, FALSE))
  ...
  period_high <- period_high %>%
    mutate(date_last_period = date - days(n))
  # date_last_period = `year<-`(date, year(date) - 1), # 求1年前的日期
  ...
  df$high_last_period <- seq_len(nrow(df)) %>%
    map_dbl(~ first(filter(period_high, date <= df$date[.x])$adjusted))
  
  df$low_last_period <- seq_len(nrow(df)) %>%
    map_dbl(~ first(filter(period_low, date <= df$date[.x])$adjusted))
